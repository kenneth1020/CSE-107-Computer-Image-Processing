...
f = imread('hong_kong.jpg'); % Read an image from file.
size(f)                      % Get size of image.
whos f                       % More info.
f(1,1)                       % Value of (1,1) pixel.
imshow(f)                    % Display image.

f_double = double(f);        % Convert image from unit8 to double. Keeps pixel range [0-255]
f_double(1,1)                % Value of (1,1) pixel.
f_double = im2double(f);     % Convert image from unit8 to double. Maps pixel range [0-255]->[0-1]
f_double(1,1)                % Value of (1,1) pixel.

imtool(f)                    % Image Viewer tool.
